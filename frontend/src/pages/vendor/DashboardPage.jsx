...
import { Package, ShoppingBag, TrendingUp, Clock, Bell } from "lucide-react";
...
const STATUS_BADGE = {
  ...
  ASSIGNED: <span className="badge-orange">Assigned</span>,
  PICKED_UP: <span className="badge-orange">Picked Up</span>,
    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      {vendor?.status === "pending" && (
        <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-2xl p-5 mb-8 flex items-start gap-4">
          ...
          <div>
            <p className="font-heading font-bold text-yellow-400">
              Account Under Review
            </p>
            <p className="font-body text-yellow-400/70 text-sm mt-1">
              Your vendor account is being verified. You can set up your
              products now — orders will start once approved (usually within 24
              hours).
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {STAT_CARDS.map((s) => (
          <div
            key={s.label}
            className={`card-dark border p-5 rounded-2xl ${s.bg}`}
          >
            <div
      ...
      <div className="grid md:grid-cols-2 gap-4 mb-8">
        ...
        >
          ...
          <div>
            <p className="font-heading font-bold text-white group-hover:text-orange-400 transition-colors">
              Manage Products
            </p>
            <p className="font-body text-white/40 text-sm">
              Add, edit, or remove your listings
            </p>
          </div>
          <span className="ml-auto text-white/20 group-hover:text-orange-400 transition-colors text-xl">
            →
          </span>
        </Link>
        <Link
      ...
      <div className="card-dark border border-white/[0.06] rounded-2xl overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.06]">
          ...
          <Link
            to="/vendor/orders"
            className="text-orange-400 text-sm font-heading hover:text-orange-300 transition-colors"
          >
            See all →
          </Link>
        </div>
        ...
        {loading ? (
          <div className="p-12 text-center text-white/20 font-body">
            Loading...
          </div>
        ) : orders.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <p className="font-body text-white/20 text-sm mt-1">
